...
class PartExitsExport implements
    FromQuery,
    WithStyles,
    WithMapping,
    WithColumnWidths,
    ...
    WithDefaultStyles
{
    ...
    {
        return PartExit::query()
            ->whereKey($this->selected)
            ->with('part', 'user', 'maintenance', 'maintenance.material')
    ...
    public function map($row): array
    {
        ...
        if (getPermissionsTeamId() === settings('site_id_verdun_siege')) {
            $data[] = $row->part->site->name;
        }

        //dd($row);

        array_push($data,
            $row->maintenance ? $row->maintenance->getKey() . ' (' . $row->maintenance?->material->name . ')' : '',
            $row->part->name,
            $row->description,
            $row->user->full_name,
            (string)$row->number,
    ...
    public function styles(Worksheet $sheet): void
    {
        ...
        $sheet->setTitle('Pièces Détachées Sorties');
