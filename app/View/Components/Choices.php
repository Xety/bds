        ...
        return <<<'HTML'
            ...
            >
                ...
                >
                    ...
                    <div @click="$refs.inputSearch.focus(); open = true" class="select select-primary w-full h-fit inline-block">
                       ...
                       <span @if($single && $searchable) x-show="!focused" @endif x-transition class="break-all">
                            <template x-for="item in selectedItems" :key="item.{{ $optionValue }}">
                                <span
                                    ...
                                    class="bg-base-200 hover:bg-base-300 rounded shadow-current px-2 py-1 mr-2 font-semibold text-sm cursor-pointer"
                        ...
                        <input
                            ...
                            @class([
                                    "outline-none focus:outline-none bg-transparent border-transparent focus:border-transparent ring-transparent focus:ring-transparent",
                                    "hidden" => !$searchable
                                ])
                                ...
                                @foreach($options as $option)
                                    ...
                                    >
                                        ...
                                        @if($item)
                                            {{ $item($option) }}
                                        @else
                                            <x-list-item :item="$option" :item-value="$optionValue" :value="$optionLabel" :sub-value="$optionSubLabel" :avatar="$optionAvatar" />
                                        @endif
                                    </div>
                                @endforeach
